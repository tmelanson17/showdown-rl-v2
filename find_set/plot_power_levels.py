...
physical = df[df['category'] == 'Physical'].sort_values('avg_pct').reset_index(drop=True)
special  = df[df['category'] == 'Special'].sort_values('avg_pct').reset_index(drop=True)

# rank -> (short label, hex color)
physical_benchmarks = {
    1640: ('Garchomp Max Earthquake',              '#E8731C'),
    1783: ('Sneasler Max Close Combat',            '#2196F3'),
    1888: ('Basculegion Adap. Inv.\n2KO Last Respects', '#9C27B0'),
    1746: ('Kangaskhan Max Last Resort',           '#4CAF50'),
}
special_benchmarks = {
    1604: ('Sylveon Max Hyper Voice',              '#E91E63'),
    1803: ('Archaludon Max Electro Shot',          '#2196F3'),
    1866: ('Charizard-MegaY Inv. Heat Wave',       '#FF9800'),
    1880: ('Charizard-MegaY Inv.\nWeather Ball',   '#F44336'),
    1904: ('Floette-Mega Inv.\nLight of Ruin',     '#9C27B0'),
}
...
def make_chart(ax, data, benchmarks, title):
    ...
    ax.annotate(
        ...
        xy=(0, y_min), xytext=(n * 0.04, y_min + y_range * 0.13),
        fontsize=7.5, fontweight='bold', color='#222222', ha='left',
    ...
    ax.annotate(
        ...
        xy=(n - 1, y_max), xytext=(n * 0.55, y_max - y_range * 0.09),
        fontsize=7.5, fontweight='bold', color='#222222', ha='left',
        arrowprops=dict(arrowstyle='->', color='#444', lw=0.9),
        bbox=dict(boxstyle='round,pad=0.25', fc='white', alpha=0.85, ec='#aaa', lw=0.5),
        zorder=5,
    ...
    for i, (idx, val, label, color) in enumerate(bench_info):
        ...
        ax.annotate(
            f'{label}\n{val:.1f}%',
            xy=(n - 1, val), xytext=(label_x, label_y[i]),
    ...
    ax.set_ylabel('Avg % Damage to 0/0 Mew', fontsize=9)
